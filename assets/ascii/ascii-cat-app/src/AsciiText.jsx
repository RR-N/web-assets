...
class AsciiFilter {
  constructor(
    ...
    /* Webflow: keep these nodes */
    this.canvas.setAttribute('data-wf-ignore', 'true');
    this.pre.setAttribute('data-wf-ignore', 'true');

    /* wrapper */
    this.dom = document.createElement('div');
    Object.assign(this.dom.style, { position: 'absolute', inset: 0 });

    /* delay append until after Webflow’s sanitizer */
    setTimeout(() => {
  ...
  setSize(w, h) {
    ...
    if (this.ctx) this.ctx.font = `${this.fontSize}px ${this.fontFamily}`;
  ...
  render(scene, camera) {
    if (!this.ctx) return;                  /* not appended yet */
    ...
    try { this.ctx.drawImage(gl, 0, 0, w, h); } catch { return; }
    let data; try { data = this.ctx.getImageData(0, 0, w, h).data; } catch { return; }

    let txt = '';
    for (let i = 0, len = data.length; i < len; i += 4) {
      const idx = i / 4, col = idx % w;
      const g   = (0.3*data[i] + 0.6*data[i+1] + 0.1*data[i+2]) / 255;
      const ci  = Math.floor((this.invert ? g : 1-g) * (this.charset.length - 1));
      txt += this.charset[ci];
      if (col === w - 1) txt += '\n';
    }
    this.pre.textContent = txt;
  }
}
